...
if(getAction().toString() == "Create") {
	...
	if( changedFieldValue != null){
		...
		// If the Epic's "Build/Run" field is not "Supporting"
			...
			getFieldByName("Eng - Department").setHidden(false).setRequired(false).setFormValue(null)
			getFieldByName("Eng - Platform").setHidden(false).setRequired(false).setFormValue(null)
...
// If this is an existing issue being edited
	...
	getFieldByName("Eng - Department").setHidden(false).setRequired(false)
	getFieldByName("Eng - Platform").setHidden(false).setRequired(false)
